    ...
    <div>
      ...
      >
        ...
        <el-form slot="editForm" :model="editForm" style="overflow: auto" label-width="120px" ref="editForm"
                 :rules="editFormRules">
          <el-form-item label="工种编号：" prop="number">
...
export default {
  ...
  data () {
    return {
      refName: 'StationCategoryForm',
      fileName: 'StationCategory.xlsx',
      url: {
        refreshUrl: '/basicCategoryData/list/stationCategory',
        searchUrl: '/basicCategoryData/list/stationCategory',
      ...
      tableHeaderList: [
        {value: 'name', label: '工位名称', width: 200},
        {value: 'number', label: '工位编号', width: 200},
  ...
  methods: {
    ...
    async addRecord () {
      this.addForm.type = 'stationCategory'
    ...
    async editRecord () {
      this.editForm.type = 'stationCategory'
    ...
    getJsonList (data) {
      for (let i = 0; i < data[0].length; i++) {
        ...
        this.jsonList[i] = {}
        this.jsonList[i].name = data[0][i]['工位名称']
        this.jsonList[i].number = data[0][i]['工位编号']
        this.jsonList[i].name = data[0][i]['简介']
        this.jsonList[i].number = data[0][i]['备注']
      }
    }
    // 批量导入时上面三个方法，一起复制。
  }

...
export default {
  ...
  data () {
    return {
      refName: 'CategoryOfPurchasedMaterialsForm',
      fileName: 'CategoryOfPurchasedMaterials.xlsx',
      url: {
        refreshUrl: '/basicCategoryData/list/categoryOfPurchasedMaterials',
        searchUrl: '/basicCategoryData/list/categoryOfPurchasedMaterials',
      ...
      tableHeaderList: [
        {value: 'name', label: '物料名称', width: 200},
  ...
  methods: {
    ...
    async addRecord () {
      this.addForm.type = 'categoryOfPurchasedMaterials'
    ...
    async editRecord () {
      this.editForm.type = 'categoryOfPurchasedMaterials'
    ...
    getJsonList (data) {
      for (let i = 0; i < data[0].length; i++) {
        ...
        this.jsonList[i].name = data[0][i]['物料名称']
      }
    }
    // 批量导入时上面三个方法，一起复制。
  }
